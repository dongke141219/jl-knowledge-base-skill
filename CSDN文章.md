...
JL Knowledge Base Skill 现在先确认当前工程确实属于杰理，再自动分类问题、查询一次相关知识、正常完成工作，最后只评估一次是否产生了新知识。不需要输入任何固定 Skill 名称、命令或指定关键词。
...
v0.7.1 暂时保持兼容，访问共享服务时会收到 v0.8.0 升级提示；但旧包没有升级器，所以这一次仍需按本文步骤手工升级。v0.8.0 是第一版具备服务器版本核对和白名单自更新能力的客户端，之后发现新版本时只运行安装包内固定升级流程，并只回报脱敏错误分类。服务器不能下发任意命令，失败后也不会让 AI 自行执行未知修复。离线旧包无法主动收到联网升级提示，本地 SDK 检查、修改和编译不受影响。
...
关键词只用于理解问题，不能证明工程属于杰理。客户端会先从目录结构、构建入口、芯片族和配置文件名等本地特征确认；证据模糊时只追问一次，确认不是杰理项目时就完全按普通 AI 流程处理。

确认后，先把客户需求分为“功能性需求”或“问题点”，再定位到产品、功能大项、能力、子功能/问题点和适用边界，然后只查询一次最相关的正式知识。命中且相关就给 AI 参考；没有命中、不相关或服务不可用，都继续正常完成本地工作。
...
第一次明确同意后，每个已确认的实质 JL 任务最多查询一次相关正式知识。AI 先把实际工作做完，结束时再评估一次：只有形成新的、可复用且有本地证据支持的工程结论，才整理至多一份小型、脱敏、结构化候选。候选经过审核后，才会进入同一个知识库的正式功能链，供以后遇到相似问题时复用。

普通闲聊、重复内容、没有可靠结论的猜测和共享知识片段原文不会上传。同一结论按规范化哈希去重；网络失败也不会循环重试、卡住回答或要求 AI 重复收口。
...
ZCode 旧版本升级时，到“设置 → 插件 → 市场源”刷新 `jl-knowledge`，再到已安装插件检查更新；更新并启用后必须新建会话。可在“设置 → Hooks”确认本插件的四类 hook 已随插件启用。
