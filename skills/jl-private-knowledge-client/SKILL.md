...
2. Call `create_knowledge_task` with one narrow, sanitized task description. Keep its returned `task_id` only for this task; never invent an ID, reuse it for another task or customer, or persist it as knowledge. Treat the returned `candidate_taxonomy` as the only valid product/domain classification for later contributions.
3. Call `query_task_fragments` with that server-issued `task_id`, the decision to make, observed behavior, and already-known evidence. Remove credentials, customer identity, private paths, complete source files, and unrelated logs.
...
1. At task start, opportunistically process up to three due outbox entries. Use the current user's authenticated MCP connection; never run or contact the knowledge owner's Codex CLI.
...
7. On offline, timeout, authentication, or rate-limit failure, schedule the local item for bounded exponential retry and leave the engineering result successful. On privacy/schema/scope rejection, remove the unsafe item rather than retrying it unchanged. If the server returns `status: withdrawn` for a previously withdrawn identical hash, drop it as `server_withdrawn`; do not retry the same rejected knowledge forever. A genuinely corrected implementation must produce a changed candidate and a new idempotency hash.
...
The current user's already-running Codex may summarize its own task into a candidate. The outbox helper performs only local JSON validation/storage, and the gateway performs only authenticated lookup/deduplication/storage. Neither component starts Codex, calls a model, has the owner's login, or spends the owner's AI usage. A GitHub or company user therefore uses their own Codex account; only workloads deliberately run by the owner's web worker use the owner's configured account.

## Failure boundaries

- If task creation fails, or the MCP dependency is unavailable or still uses the non-routable placeholder, explain that private knowledge was not queried and continue from local project evidence. Do not call query or submit without a server-issued `task_id`.
...
- Do not bypass authentication, retry authentication failures with guessed credentials, fall back to direct NAS/file access, or launch a remote Codex worker.
